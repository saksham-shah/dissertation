...
def evaluate(config, model, test_loader, q_lang, a_lang):
    ...
    for mwp in test_loader:
        q_tensor = tensorFromTokens(q_lang.token2index, mwp['question'][0].split(" "))
        ...
        for equation in all_equations:
            ...
            a_tensor = tensorFromTokens(a_lang.token2index, equation.split(" "))
...
def train(config, model, train_loader, optimiser, criterion, q_lang, a_lang):
    ...
    for mwp in train_loader:
        ...
        for equation in all_equations:
            ...
            # if random.random() < force_correct:
            #     a_indexes = [indexesFromTokens(a_lang.token2index, a.split(" ")) for a in mwp['formula']]
            #     target = [1.0 for _ in mwp['formula']]
            # else:
            #     a_indexes = [indexesFromTokens(a_lang.token2index, equation.split(" ")) for _ in mwp['formula']]
            #     target = [1.0 if is_equivalent(a, equation) else 0.0 for a in mwp['formula']]
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    end_time = timer()
    # val_loss = evaluate(config, classifier, test_loader, criterion)
